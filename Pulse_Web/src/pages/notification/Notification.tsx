...
const Notification = () => {
  ...
  const [filter, setFilter] = useState<"all" | "message" | "like" | "comment">("all");
      ...
      <main className="relative z-10 p-4 text-white h-full overflow-y-auto">
        ...
        <div className="bg-zinc-900 p-4 rounded-lg shadow-lg flex items-center justify-between">
          <div className="flex gap-4">
            {["all", "message", "like", "comment"].map((type) => (
              <button
                key={type}
                onClick={() => setFilter(type as any)}
                className={`px-4 py-2 rounded-lg transition font-semibold focus:outline-none ${
                  filter === type ? "bg-white text-black" : "text-gray-400 hover:bg-white/10 cursor-pointer"
                }`}
              >
            ...
            filteredNotifications.map((notification) => (
              <div
                ...
                onClick={async () => {
                  ...
                  navigate("/page-2"); // 📌 chỉnh thành route bạn muốn chuyển tới
                }}
              >
